...
def rolling_return(
    ...
    if not snap_to_closest:
        simple_return = (
            s_filled
            .pct_change(freq=offset, fill_method=None)
            .add(1)
        )
...
def normalize_currencies(df : pd.DataFrame, to="USD"):
    """Assume df is a dataframe that has: 1. OHCLV columns or 2. Multilevel index"""
    # for each ticker, check currency and convert to USD if necessary
    # we convert each of OHLC & Adj Close. Volume (and other columns are preserved)
    ccy_cols = ["Open", "High", "Low", "Close", "Adj Close"]
    df.columns.levels
    # for ticker in tickers:
    #     ccy = yf.Ticker(ticker).fast_info["currency"]
    #     if ccy != "USD":
    #         ccy_pair = download_ccy_pair(ccy_from=ccy, start=start_date, end=end_date)["Close"]
    #         for col in ccy_cols:
    #             df[(col, "f{ticker}_USD")] = df[(col, "...")
